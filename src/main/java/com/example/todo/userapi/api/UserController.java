...
public class UserController {
    ...
    public ResponseEntity<?> check(String email) {
        ...
        boolean resultFlag = userService.isDuplicate(email); //중복되었니?
    ...
    public ResponseEntity<?> signup(
            ...
            @RequestPart(value = "profileImage", required = false) MultipartFile profileImg, //required =false 이미지를 첨부하지 않은 사람
    ...
    public ResponseEntity<?> signIn(
            @Validated @RequestBody LoginRequestDTO dto   //json으로 넘어오는 데이터는 @requestbody로 자바객체로 변환.
    ) {
        try {
    ...
    @PreAuthorize("hasRole('ROLE_COMMON')")  // ROLE_COMMON이 아니라면 실행 x
    ...
    public ResponseEntity<?> loadFile(
        ...
        try {
            ...
            if(!profileFile.exists()) {
                return ResponseEntity.notFound().build(); //403
            }
            ...
            MediaType contentType = findExtensionAndGetMediaType(filePath); //findexten 커서 ctrl+alt+v
            if(contentType == null) {
    ...
    private MediaType findExtensionAndGetMediaType(String filePath) {
        ...
        // 파일 경로에서 확장자 추출하기
        // C:/todo_upload/asdsadadssaf_abc.jpg
        String ext
